...
def send_mono(device, command):
	try:
		...
		return f'{device["host"]} time out\n'
	except netmiko.NetmikoAuthenticationException:
		return f'{device["host"]} auth fail\n'

def send_command_to_devices(devices, commands_dict, filename, limit=3):
	with concurrent.futures.ThreadPoolExecutor(max_workers=limit) as executor:
		futures = [
			executor.submit(send_mono, device, commands_dict[device["host"]])
...
if __name__ == '__main__':
	...
	send_command_to_devices(devices, commands, 'task_19_3.txt')